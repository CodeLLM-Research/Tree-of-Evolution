...
def parse_arguments() -> argparse.Namespace:
    """Parse command-line arguments for the complexity scoring script."""
    parser = argparse.ArgumentParser(
        description="Process programming questions using LLM for complexity scoring."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--temperature", type=float, default=0.0, help="Temperature for the model response (default: 0.0)."
    )
    parser.add_argument(
...
def extract_data(input_text: str) -> Optional[float]:
    """
    Extract the complexity score from the model's response.

    Args:
        input_text (str): The raw response from the model.

    Returns:
        Optional[float]: The extracted complexity score, or None if not found.
    """
    score = None
    if "Step 2 [Final Score]:" in input_text:
        try:
            ...
            logger.warning("Failed to parse score from response: %s", input_text)
            score = None
...
def process_item(
    ...
    """
    Process a single item by evaluating programming questions and assigning complexity scores.

    Args:
        client (OpenAIClient): The LLM client.
        obj (Dict[str, Any]): The input object containing programming questions.
    ...
    try:
        ...
        logger.error("Error rendering prompt for item %s: %s", idx, e)
        return None
    ...
    try:
        ...
        if score is not None:
            ...
            logger.warning("No valid complexity score extracted for item %s.", idx)
    except Exception as e:
        logger.error("Error processing item %s: %s", idx, e)
        return None
...
def main() -> None:
    """Main entry point for the complexity scoring script."""
    # Configure logging
    logging.basicConfig(
        level=logging.INFO,
    ...
    # Load Jinja2 template for complexity judging
